        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          {STACK.map((s) => (
            <div key={s.name} className="rounded-lg border border-line bg-bg-raised p-3">
              <div className="text-[13px] font-semibold text-text">{s.name}</div>
              <div className="mt-0.5 text-[11.5px] leading-snug text-text-3">{s.role}</div>
            </div>
          ))}
        </div>
        <p className="mt-5 max-w-2xl text-[12px] leading-relaxed text-text-3">
