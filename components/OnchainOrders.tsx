    ...
    <div className="flex items-center justify-between gap-3 px-4 py-3">
      <div className="min-w-0">
        <div className="flex items-center gap-2">
          <span className="text-[13px] font-medium text-text">{d.text}</span>
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="mono mt-0.5 inline-block text-[10.5px] text-text-4 hover:text-violet"
        >
      ...
      <div className="text-right">
        <Eyebrow>at</Eyebrow>
        <div className="num text-[13px] text-text">{usd(d.price)}</div>
      </div>
    </div>
  );
}
    ...
    >
      ...
      {hasProof() && (
        <div className="border-b border-violet/25 bg-violet/5 px-4 py-3">
          ...
          {PROOF.summary && <div className="mt-1 text-[11.5px] text-text-2">{PROOF.summary}</div>}
            ...
            {PROOF.order ? (
              <>
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="mono inline-flex items-center gap-1 rounded border border-violet/40 bg-violet/15 px-2 py-1 text-[11px] text-violet hover:bg-violet/25"
                >
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="mono inline-flex items-center gap-1 text-[11px] text-text-4 hover:text-violet"
                >
                  Solscan ↗
                </a>
              </>
          ...
          {PROOF.maker && (
            <div className="mono mt-1.5 text-[10px] text-text-4">
      ...
      {address && orders.length === 0 && !hasProof() && (
        <div className="px-4 py-5 text-[12.5px] text-text-3">
          No resting orders on this wallet. Arm a take profit or buy the dip to place one.
        </div>
      )}
      {!address && !hasProof() && (
        <div className="px-4 py-5 text-[12.5px] leading-relaxed text-text-3">
      ...
      <div className="border-t border-line px-4 py-3">
        ...
        <div className="mt-2 flex items-center gap-2">
          <input
            ...
            spellCheck={false}
            className="mono min-w-0 flex-1 rounded-lg border border-line-strong bg-bg-inset px-2.5 py-1.5 text-[12px] text-text outline-none placeholder:text-text-4"
          />
          {address && (
        ...
        {probe !== null && !err && (
          <div className="mt-2 overflow-hidden rounded-lg border border-line">
            {probe.length === 0 ? (
              <div className="px-3 py-2.5 text-[11.5px] text-text-3">
